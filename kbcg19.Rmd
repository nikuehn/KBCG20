...
* `k1` and `k2`: parameters needed fr site amplification
...
kbcg19 <- function(period, m, rlist, ztor, fs, vs, z_1, z_2_5, fx, mb, reg) {
...
kbcg19_posterior <- function(period, m, rlist, ztor, fs, vs, z_1, z_2_5, fx, mb, reg, num_samples = 100) {
  # need to add some checks for input (period, region)
  ...
  # calculate PSA
  parameters_posterior <- read.csv(sprintf('/Users/nico/GROUNDMOTION/PROJECTS/SUBDUCTION/CBGK/STAN_V16/RESULTS/CSV_allregca_attn3_corrreg_cs_dmb/coefficients_T%05.2f.csv',period))
  k1k2 <- interp_k1k2(period)
  dmb <- interp_dmb(period)
