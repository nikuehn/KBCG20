...
model {
  ...
  for(i in 1:N) {
    ...
    // base model
    mu1 = (1 - FS[i]) * (par1_reg[reg[i]] + logistic_hinge(M[i],mbreak_if[i],par4 * (mbreak_if[i] - refmb_lh),par4,par5,delta_m));
    ...
    // site amplification
    siterock = (par7_reg[reg[i]] + k2n) * lnVSrock_k1;
    pga_rock = exp(mu1 + mu1a + mu2 + mu4 + mu5 + siterock + mu_geom + mu_geoma + eqterm[eq[i]]);



    if(VS[i] < k1) {
      mu3 = par7_reg[reg[i]] * lnVSk1[i] + k2 * (log(pga_rock + cVSk1n[i]) - log(pga_rock + c));
    }
    else {
