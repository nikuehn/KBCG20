...
knitr::opts_chunk$set(echo = TRUE, fig.width=12,fig.height = 8, fig.path = 'pictures/',root.dir = '/Users/nico/GROUNDMOTION/PROJECTS/SUBDUCTION/CBGK/STAN_V16/FUNC/')
```
...
```{r define_helper_functions, attr.source='.numberLines'}
...
```{r func_median_prediction, attr.source='.numberLines'}
# function to calculate median prediction
...
library(knitr)
mb <- read.csv('DATA/kbcg_mbreak_regional.csv')
kable(mb, caption = 'Magnitude Break points')
```


```{r func_median_wrapper, attr.source='.numberLines'}
...
```{r func_median_distribution, attr.source='.numberLines'}
...
```{r func_arc_distances, attr.source='.numberLines'}
